...
class SnakeGame:
    def __init__(self, ai_snake=False, ai_function=None, snake=None, config={}):
        # Comprobaciones
        if ai_snake == True and ai_function is None:
        ...
        self.food_image = pygame.transform.scale(pygame.image.load(os.path.join(SOURCE_PATH, "cereza.png")),(self.block, self.block))
        ...
        while self.run_loop:
            ...
            if not self.run_game:
                text_start = self.font_2.render('Presiona R para comenzar', True,self.config["colors"]["white"], self.config["colors"]["black"])
                self.screen.blit(text_start,(self.width_window - 180, 50))
            else:
                text_start = self.font_2.render('', True,self.config["colors"]["white"], self.config["colors"]["black"])
                self.screen.blit(text_start,(self.width_window - 180, 50))
            text_title = self.font.render('SNAKE GAME', True, self.config["colors"]["white"], self.config["colors"]["black"])
            self.screen.blit(text_title,(self.width_window - 180, 10))
            text_score = self.font_2.render(f"Score: {self.count_food}", True, self.config["colors"]["white"], self.config["colors"]["black"])
            self.screen.blit(text_score,(self.width_window - 180, 70))
            if self.data_state["STATUS"] == "GAMEOVER":
                text_game_over = self.font.render('GAME OVER', True, self.config["colors"]["white"], self.config["colors"]["black"])
                self.screen.blit(text_game_over,(self.width_game // 2 - 60, self.height_game // 2))
            elif self.data_state["STATUS"] == "WINNER":
                text_game_over = self.font.render('GANASTE', True, self.config["colors"]["white"], self.config["colors"]["black"])
                self.screen.blit(text_game_over,(self.width_game // 2 - 60, self.height_game // 2))
    ...
    def loop_game(self, snake, ai_function, speed, data_state={}):
        ...
        food_block = Food(
            ...
            round(random.randrange(0, self.height_game - self.block) / self.block) * self.block, self.block, 
            self.block, 
            self.food_image, 
            self.block)
        ...
        while self.run_game:
            ...
            if self.pause:
                text_pause = self.font.render('PAUSA', True, self.config["colors"]["white"], self.config["colors"]["black"])
                self.screen.blit(text_pause,(self.width_game // 2 - 20, self.height_game // 2))
            else:
                text_pause = self.font.render('', True, self.config["colors"]["white"], self.config["colors"]["black"])
                self.screen.blit(text_pause,(self.width_game // 2 - 20, self.height_game // 2))
